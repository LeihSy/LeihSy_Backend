...
import com.hse.leihsy.model.dto.ItemBatchCreateDTO;
...
public class ItemController {
    ...
    /**
     * Endpoint für Batch-Erstellung (Sets)
     */
    @PostMapping("/batch")
    public ResponseEntity<List<ItemDTO>> createItemBatch(@Valid @RequestBody ItemBatchCreateDTO batchDTO) {
        // Aufruf der Service-Logik mit automatischer Nummerierung
        List<Item> items = itemService.createItemSet(
                batchDTO.getInvNumberPrefix(),
                batchDTO.getOwner(),
                batchDTO.getProductId(),
                batchDTO.getCount()
        );

        return ResponseEntity.status(HttpStatus.CREATED).body(itemMapper.toDTOs(items));
    }
